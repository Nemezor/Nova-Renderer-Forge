...
public class NovaRenderer implements IResourceManagerReloadListener {
    ...
    public void addTerrainAtlas(@Nonnull TextureMap blockColorMap) {
        // Copy over the atlas
        NovaNative.mc_atlas_texture blockColorTexture = getFullImage(Interface.textureMap_getWidth(blockColorMap), Interface.textureMap_getHeight(blockColorMap), Interface.textureMap_getMapUploadedSprites(blockColorMap).values());
        ...
        for(String spriteName : Interface.textureMap_getMapUploadedSprites(blockColorMap).keySet()) {
    ...
    public void updateCameraAndRender(float renderPartialTicks, long systemNanoTime, Minecraft mc) {
        ...
        boolean shouldUpdateLightmap = Interface.entityRenderer_isLightmapUpdateNeeded(entityRenderer);
        Interface.entityRenderer_updateLightmap(entityRenderer, renderPartialTicks);
        if(shouldUpdateLightmap) {
            sendLightmapTexture(Interface.entityRenderer_getLightmap(entityRenderer));
        }
    ...
    private void sendLightmapTexture(DynamicTexture lightmapTexture) {
        ...
        NovaNative.INSTANCE.send_lightmap_texture(data, data.length, Interface.dynamicTexture_getWidth(lightmapTexture), Interface.dynamicTexture_getHeight(lightmapTexture));
    }
